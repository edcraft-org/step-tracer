...
class TracerTransformer(ast.NodeTransformer):
    ...
    def __init__(self) -> None:
        ...
        self.utils_name = "_step_tracer_utils"
    ...
    def _name(self, name: str, ctx: ast.expr_context | None = None) -> ast.Name:
        return ast.Name(id=name, ctx=ctx or ast.Load())

    def _attr(self, value: ast.expr, attr: str) -> ast.Attribute:
        return ast.Attribute(value=value, attr=attr, ctx=ast.Load())

    def _call(self, func: ast.expr, args: list[ast.expr]) -> ast.Call:
        return ast.Call(func=func, args=args, keywords=[])

    def _exec_ctx_attr(self, attr: str) -> ast.Attribute:
        return self._attr(self._name(self.exec_ctx_name), attr)

    def _current_exec_attr(self, attr: str) -> ast.Attribute:
        return self._attr(self._exec_ctx_attr("current_execution"), attr)

    def _safe_copy(self, expr: ast.expr) -> ast.Call:
        return self._call(
            self._attr(self._name(self.utils_name), "safe_deepcopy"),
            [expr],
        )
    ...
    def visit_For(self, node: ast.For) -> list[ast.stmt]:
        ...
        for var_name, access_path in variables:
            node.body.insert(
                0,
                self._create_variable_tracking_call(var_name, access_path, node.lineno),
            )
    ...
    def _create_loop_exec_call(self, lineno: int, loop_type: str) -> ast.Call:
        return self._call(
            self._exec_ctx_attr("create_loop_execution"),
            [ast.Constant(lineno), ast.Constant(loop_type)],
        )

    def _create_loop_iter_call(self) -> ast.Call:
        return self._call(
            self._exec_ctx_attr("create_loop_iteration"),
            [],
        )
    ...
    def _transform_func_def(self, node: _FuncDef) -> _FuncDef:
        ...
        new_body: list[ast.stmt] = [
            self._create_reset_args_call(),
            self._create_set_func_def_line_num_call(node.lineno),
        ]
        ...
        for arg in all_args:
            arg_expr = self._name(arg.arg)

            new_body.append(self._create_add_arg_call(arg.arg, arg_expr))

            new_body.append(
                self._create_variable_tracking_call(arg.arg, arg.arg, node.lineno)
            )
    ...
    def _expand_call(self, node: ast.Call) -> tuple[ast.With, ast.Expr]:
        ...
        temp_node = ast.Assign(
            targets=[self._name(tmp_ret_var, ast.Store())],
            value=node,
        )

        record_ret_node = self._create_record_func_return_call(self._name(tmp_ret_var))

        new_node = ast.Expr(self._name(tmp_ret_var))

        return (
            self._wrap_with_ctx(
                [*arg_stmts, temp_node, record_ret_node],
                create_func_call,
            ),
            new_node,
        )
    ...
    def _create_variable_tracking_call(
        ...
        return ast.parse(
            f"{self.exec_ctx_name}.{method}({var_name!r}, "
            f"{self.utils_name}.safe_deepcopy({var_name}), "
            f"{access_path!r}, {line_no})"
        ).body[0]
    ...
    def visit_If(self, node: ast.If) -> list[ast.stmt]:
        ...
        temp_name = self._get_temp_var_name()

        temp_assign = ast.Assign(
            targets=[self._name(temp_name, ast.Store())],
            value=node.test,
        )
    ...
    def _wrap_with_ctx(
        ...
        return ast.With(
            items=[
                ast.withitem(
                    context_expr=self._call(
                        self._exec_ctx_attr("track_stmt_exec"),
                        [create_stmt_exec_node],
                    )
                )
            ],
            body=body,
    ...
    def _get_func_full_name(self, func: ast.expr) -> str:
        if isinstance(func, ast.Name):
            ...
            return f"{self._get_func_full_name(func.value)}.{func.attr}"
        return "<lambda_or_unknown>"

    def _find_global_nonlocal(self, node: ast.AST, g: set[str], n: set[str]) -> None:
        for child in ast.iter_child_nodes(node):
            ...
            if isinstance(child, ast.Global):
                ...
                n.update(child.names)
            else:
                self._find_global_nonlocal(child, g, n)
    ...
    def _extract_variable_names(self, target: ast.expr) -> list[tuple[str, str]]:
        ...
        if isinstance(target, ast.Name):
            ...
            variables.extend(self._extract_variable_names(target.value))
    ...
    def _get_base_name(self, node: ast.expr) -> tuple[str, str] | None:
        if isinstance(node, ast.Name):
            ...
            if base:
                return base[0], f"{base[1]}.{node.attr}"
        elif isinstance(node, ast.Subscript):
            base = self._get_base_name(node.value)
            if base:
                return base[0], f"{base[1]}[{ast.unparse(node.slice)}]"
        return None
    ...
    def _create_func_call(
        ...
        return self._call(
            self._exec_ctx_attr("create_function_call"),
            [
                ast.Constant(node.lineno),
                ast.Constant(func_name),
                ast.Constant(func_full_name),
            ],
        )
    ...
    def _create_add_arg_call(self, name: str, arg_value: ast.expr) -> ast.Expr:
        return ast.Expr(
            value=self._call(
                self._current_exec_attr("add_arg"),
                [ast.Constant(name), self._safe_copy(arg_value)],
            )
        )
    ...
    def _create_record_func_return_call(self, return_value: ast.expr) -> ast.Expr:
        return ast.Expr(
            value=self._call(
                self._current_exec_attr("set_return_value"),
                [self._safe_copy(return_value)],
            )
        )
    ...
    def _create_set_func_def_line_num_call(self, line_num: int) -> ast.Expr:
        return ast.Expr(
            value=self._call(
                self._current_exec_attr("set_func_def_line_num"),
                [ast.Constant(line_num)],
            )
        )

    def _create_reset_args_call(self) -> ast.Expr:
        return ast.Expr(value=self._call(self._current_exec_attr("reset_args"), []))
